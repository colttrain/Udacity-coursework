...
def create_spark_session():
    """
    Overview:
    - Connects to Spark and creates a session
    - Programmatically attaches my keys using sparkContext
    Param: None
    Return: None
    """
    spark = SparkSession \
...
def process_log_data(spark, input_data, output_data):
    ...
    Params: spark(creates SparkSession), input_data(udacity s3 filepath) and output_data(student s3 filepath)
    Returns: None
    """
    ...
    user_table = df.select((df.userId).alias('user_id'), 
    ...
    songplays.write.partitionBy('year', 'month').parquet(os.path.join(output_data, 'songplays'), 'overwrite')
