...
songplay_table_create = """CREATE TABLE IF NOT EXISTS songplays (songplay_id SERIAL PRIMARY KEY, start_time timestamp NOT NULL, user_id int, level varchar, song_id varchar, artist_id varchar, session_id int NOT NULL, location varchar, user_agent varchar, UNIQUE (start_time, user_id, session_id))"""

user_table_create = "CREATE TABLE IF NOT EXISTS users (user_id varchar PRIMARY KEY, first_name varchar, last_name varchar, gender text, level varchar)"
...
songplay_table_insert = "INSERT INTO songplays (start_time, level, song_id, artist_id, session_id, location, user_agent) VALUES (%s, %s, %s, %s, %s, %s, %s)"
